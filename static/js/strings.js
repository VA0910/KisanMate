/**
 * Single source of truth for every UI string, keyed by language.
 * Adding a language = adding one more top-level key here (plus a locale
 * tag and a language-picker card) -- nothing else in the app changes.
 */
(function (global) {
  "use strict";

  var LANGUAGES = [
    { code: "en", label: "English" },
    { code: "hi", label: "हिंदी" },
    { code: "te", label: "తెలుగు" }
  ];

  // BCP-47 tags used for both SpeechRecognition.lang and SpeechSynthesisUtterance.lang.
  var LOCALE_TAGS = { en: "en-IN", hi: "hi-IN", te: "te-IN" };

  // Shown on the language picker itself, before we know which language to use.
  var CHOOSE_LANGUAGE_HEADING = ["Choose your language", "अपनी भाषा चुनें", "మీ భాషను ఎంచుకోండి"];

  var STRINGS = {
    en: {
      appName: "KisanMate",
      tagline: "Your farm helper",
      skipToContent: "Skip to main content",
      changeLanguage: "Change language",
      muteVoice: "Mute voice replies",
      unmuteVoice: "Unmute voice replies",
      govtSchemes: "Government Schemes",
      govtSchemesMenuLabel: "Government schemes menu",
      govtSchemePmKisan: "PM-KISAN (Kisan Nidhi)",
      govtSchemePmfby: "Crop Insurance (Kisan Beema)",
      govtSchemeSoilHealth: "Soil Health Card",
      back: "Back",
      navHome: "Home",
      navDiagnose: "Diagnose",
      navReports: "Reports",
      navAlerts: "Alerts",
      retry: "Try again",
      cancel: "Cancel",
      loading: "Loading...",
      friendlyErrorGeneric: "Something went wrong. Please try again.",
      friendlyErrorSlow: "That's taking longer than usual. Please check your connection and try again.",

      getStarted: "Get started",
      continueLabel: "Continue",
      continueBtn: "Continue to home",
      signOut: "Sign out",
      signedOut: "You've been signed out.",
      sessionExpired: "Your session is no longer valid. Please sign in again.",
      phoneTitle: "Sign in with your phone",
      phoneInstruction: "Enter your phone number and we'll send you a code.",
      phoneLabel: "Phone number",
      phonePlaceholder: "10-digit mobile number",
      phoneInvalid: "Please enter your 10-digit mobile number.",
      sendCode: "Send code",
      otpRequestError: "We couldn't send the code right now. Please try again.",
      otpTitle: "Enter the code",
      otpSentTo: function (phone) { return "We sent a code to " + phone + "."; },
      demoOtpLabel: "Demo OTP",
      otpLabel: "4-digit code",
      otpInvalid: "That code doesn't match. Please try again.",
      verifyCode: "Verify & continue",
      changeNumber: "Change number",
      profileSetupTitle: "Set up your profile",
      profileSetupIntro: "Welcome! Tell us your name to get started. You can add your farm details next.",
      profileNameLabel: "Your name",
      profileNamePlaceholder: "e.g. Ramesh",

      setupTitle: "Let's set up your farm",
      setupStepOf: function (n, total) { return "Step " + n + " of " + total; },
      locationQuestion: "Where is your farm?",
      locationHelp: "We use this to warn you about risks near you.",
      useMyLocation: "Use my location",
      locationDetecting: "Finding your location…",
      locationDeviceSet: "Using your current location.",
      locationDistrictSet: function (d) { return "Set to " + d + "."; },
      locationDenied: "We couldn't get your location. Please pick your district below.",
      districtLabel: "Or type your village / town / district",
      districtPlaceholder: "Type a place, e.g. Kanpur",
      searchingPlaces: "Searching…",
      noPlaceMatches: "No matching places. Keep typing.",
      next: "Next",
      soilQuestion: "What is your soil like?",
      soilRequired: "Please choose your soil type.",
      cropsQuestion: "What are you growing now?",
      currentCropsHelp: "Add each crop and the date you planted it.",
      currentCropsLabel: "Your crops",
      addCrop: "Add a crop",
      selectCropPlaceholder: "Choose a crop",
      plantingDateLabel: "Planting date",
      cropRemove: "Remove crop",
      cropsRequired: "Please add at least one crop.",
      cropDateRequired: "Please set a planting date for each crop.",
      finishSetup: "Finish",
      profileTitle: "Your profile",
      languageLabel: "Language",
      locationLabel: "Location",
      saveProfile: "Save changes",
      profileSaved: "Profile saved.",
      profileSaveError: "We couldn't save your profile. Please try again.",

      remindersTitle: "Reminders for you",
      dueToday: "Due today",
      reminderIrrigateToday: function (crop) { return "Time to irrigate your " + crop + "."; },
      reminderIrrigateIn: function (crop, n) { return "Irrigate your " + crop + " in " + n + (n === 1 ? " day." : " days."); },
      reminderIrrigateDrySpell: function (crop) { return "No rain expected for 7 days — irrigate your " + crop + "."; },
      reminderHarvestNow: function (crop) { return "Harvest time for your " + crop + "."; },
      reminderHarvestIn: function (crop, n) { return "Harvest window for your " + crop + " in " + n + (n === 1 ? " day." : " days."); },
      reminderStageCare: function (crop) { return "Your " + crop + ":"; },

      homeGreeting: "Welcome",
      assistantWelcome: "🙏 Namaste — how can I help you today?",
      tapAndSpeak: "Tap and speak",
      listening: "Listening...",
      micUnavailableHint: "Voice isn't available on this device. Use the text box below.",
      micPermissionDenied: "Please allow microphone access, or type your question below.",
      voiceNotUnderstood: "Sorry, I didn't catch that. Please try again or type your question below.",
      diagnoseCardTitle: "Diagnose my crop",
      diagnoseCardSub: "Take a photo of your plant",
      assistantQuestionPlaceholder: "e.g. what should I grow after tomatoes?",
      assistantYouAsked: function (q) { return "You asked: " + q; },
      assistantAskEmpty: "Please ask a question first.",
      assistantError: "I couldn't answer that right now. Please try again.",
      chipGrow: "What should I grow?",
      chipMandi: "Today's tomato price",
      chipFertilizer: "Which fertiliser for my crop?",
      chipWeather: "Will it rain this week?",

      diagnoseInstruction: "Take or choose a clear photo of the leaf",
      addPhoto: "Add Photo",
      takePhoto: "Take photo",
      uploadPhoto: "Upload",
      capturePhoto: "Capture",
      cameraLivePreview: "Live camera preview",
      photoPreviewAlt: "Photo you selected",
      changePhoto: "Choose a different photo",
      submitDiagnose: "Check my crop",
      thinking: "Thinking...",
      thinkingSub: "This can take a few seconds",
      confidenceLow: "Low",
      confidenceMedium: "Medium",
      confidenceHigh: "High",
      confidenceUncertain: "Not sure yet",
      confidenceLabel: "How sure we are",
      adviceLabel: "What to do",
      explainWhat: "What this is",
      explainWhy: "Why",
      explainWhatToDo: "What to do",
      statusAdvise: "We found this",
      statusEscalate: "Needs a closer look",
      playAudio: "Play",
      newPhoto: "New Photo",
      notRight: "This isn't right",
      disputeConfirmQuestion: "Send this to your local RSK officer for a second look?",
      disputeThanks: "Thank you. We've told your local RSK officer to take a look.",
      diagnoseError: "We couldn't check your photo right now. Please try again.",

      recommendInstruction: "Tell us about your field",
      soilLabel: "Soil type",
      zoneLabel: "Your area",
      rainfallLabel: "Rainfall",
      groundwaterLabel: "Water depth",
      chooseAllHint: "Please choose one option in each group",
      showRecommendations: "Show suggestions",
      bestMatch: "Best match",
      startOver: "Start over",
      recommendResultIntro: "Here are good crops for your field",
      orTypeQuestion: "Or type your question",
      askBtn: "Ask",
      askAgain: "Ask again",

      alertsTitle: "Your Alerts",
      alertsEmpty: "No alerts right now. We'll let you know if there's a risk near your farm.",
      alertsLoadError: "We couldn't load your alerts. Please check your connection.",
      inYourArea: "in your area",
      withinKm: function (km) { return "within " + km + " km"; },
      today: "Today",
      yesterday: "Yesterday",
      daysAgo: function (n) { return n + " days ago"; },
      alertsSummarySpoken: function (n) {
        return n === 1 ? "You have 1 new alert" : "You have " + n + " new alerts";
      },

      reportsTitle: "Your Reports",
      reportsEmpty: "You haven't diagnosed a crop yet.",
      reportsLoadError: "We couldn't load your reports. Please check your connection.",
      officerReviewed: "Reviewed by RSK officer",
      reportStatuses: {
        pending: "Checking...",
        advised: "AI diagnosis",
        escalated: "Awaiting officer review",
        disputed: "You flagged this",
        confirmed: "Confirmed by officer"
      },
      verdictModalTitle: "An officer reviewed your crop",
      verdictModalBody: function (condition) {
        return "A local RSK officer has reviewed your case. Their finding: " + condition + ".";
      },
      verdictModalBodyMany: function (n) {
        return "A local RSK officer has reviewed " + n + " of your cases. Tap below to see the details.";
      },
      verdictModalView: "See details",
      verdictModalDismiss: "Close",

      conditions: {
        late_blight: "Late Blight",
        early_blight: "Early Blight",
        nitrogen_deficiency: "Low Nitrogen",
        other: "Other Issue",
        healthy: "Healthy"
      },
      crops: {
        rice: "Rice", tomato: "Tomato", chili: "Chili",
        cotton: "Cotton", groundnut: "Groundnut", maize: "Maize"
      },
      reasons: {
        high_rainfall_shallow_water_suits_paddy: "Good rain and shallow water suit this crop",
        moderate_water_well_drained_soil_suits_tomato: "Well-drained soil suits this crop",
        well_drained_soil_moderate_rainfall_suits_chili: "Well-drained soil and moderate rain suit this crop",
        black_soil_semi_arid_zone_suits_cotton: "Black soil in a dry area suits this crop",
        low_water_need_suits_deep_groundwater_zones: "Needs little water, good for your area",
        adaptable_crop_moderate_rainfall_suits_maize: "An easy crop that grows well with moderate rain"
      },
      soils: {
        alluvial: "Alluvial (river) soil", black: "Black soil", loamy: "Loamy soil",
        loam: "Loam soil", clay: "Clay soil",
        red: "Red soil", sandy: "Sandy soil"
      },
      zones: { delta: "Delta", coastal: "Coastal", upland: "Upland", semi_arid: "Dry area" },
      rainfall: { low: "Low rain", medium: "Medium rain", high: "High rain" },
      groundwater: { shallow: "Shallow", medium: "Medium", deep: "Deep" },
      tiers: { watch: "Watch", warning: "Warning", alert: "Alert" },

      demoSectionTitle: "Demo",
      resetDemo: "Reset demo data",
      resetDemoDone: "Demo data reset to a clean state.",
      resetDemoError: "Couldn't reset the demo data. Please try again.",

      skipIntro: "Skip intro",
      watchIntro: "Watch intro",
      officerSignIn: "RSK officer sign-in",
      introWarned: "Warned",
      introSafe: "Not at risk",
      introDxNote: "The photo shows early spots, but the cool, damp weather points to late blight — two different diseases. Rather than guess, KisanMate sends it to a local officer.",
      introScene1: "This is Ramesh, a tomato farmer in Guntur.",
      introScene2: "His crop looks unwell, so he sends a photo.",
      introScene3: "KisanMate isn't certain between two diseases, so instead of guessing it sends the case to the local officer.",
      introScene4: "An agricultural officer reviews it and confirms late blight.",
      introScene5: "Because it spreads, nearby tomato farmers are warned instantly. The rice farmer next door isn't — only those actually at risk.",
      introScene6: "This is KisanMate. Let's begin."
    },

    hi: {
      appName: "KisanMate",
      tagline: "आपकी खेती में मदद",
      skipToContent: "मुख्य सामग्री पर जाएं",
      changeLanguage: "भाषा बदलें",
      muteVoice: "आवाज़ में जवाब बंद करें",
      unmuteVoice: "आवाज़ में जवाब शुरू करें",
      govtSchemes: "सरकारी योजनाएं",
      govtSchemesMenuLabel: "सरकारी योजनाओं की सूची",
      govtSchemePmKisan: "पीएम-किसान (किसान निधि)",
      govtSchemePmfby: "फसल बीमा (किसान बीमा)",
      govtSchemeSoilHealth: "मृदा स्वास्थ्य कार्ड",
      back: "पीछे",
      navHome: "होम",
      navDiagnose: "जांच",
      navReports: "रिपोर्ट",
      navAlerts: "सूचना",
      retry: "फिर से कोशिश करें",
      cancel: "रद्द करें",
      loading: "लोड हो रहा है...",
      friendlyErrorGeneric: "कुछ गड़बड़ हो गई। कृपया फिर से कोशिश करें।",
      friendlyErrorSlow: "इसमें सामान्य से ज़्यादा समय लग रहा है। कृपया इंटरनेट जांचें और फिर से कोशिश करें।",

      getStarted: "शुरू करें",
      continueLabel: "आगे बढ़ें",
      continueBtn: "होम पर जाएं",
      signOut: "साइन आउट",
      signedOut: "आप साइन आउट हो गए हैं।",
      sessionExpired: "आपका सत्र मान्य नहीं रहा। कृपया फिर से साइन इन करें।",
      phoneTitle: "अपने फ़ोन से साइन इन करें",
      phoneInstruction: "अपना फ़ोन नंबर डालें, हम आपको एक कोड भेजेंगे।",
      phoneLabel: "फ़ोन नंबर",
      phonePlaceholder: "10 अंकों का मोबाइल नंबर",
      phoneInvalid: "कृपया अपना 10 अंकों का मोबाइल नंबर डालें।",
      sendCode: "कोड भेजें",
      otpRequestError: "अभी कोड नहीं भेज पाए। कृपया फिर से कोशिश करें।",
      otpTitle: "कोड डालें",
      otpSentTo: function (phone) { return "हमने " + phone + " पर एक कोड भेजा है।"; },
      demoOtpLabel: "डेमो OTP",
      otpLabel: "4 अंकों का कोड",
      otpInvalid: "यह कोड मेल नहीं खाता। कृपया फिर से कोशिश करें।",
      verifyCode: "पुष्टि करें और आगे बढ़ें",
      changeNumber: "नंबर बदलें",
      profileSetupTitle: "अपनी प्रोफ़ाइल बनाएं",
      profileSetupIntro: "स्वागत है! शुरू करने के लिए अपना नाम बताएं। खेत की जानकारी आप आगे जोड़ सकते हैं।",
      profileNameLabel: "आपका नाम",
      profileNamePlaceholder: "जैसे रमेश",

      setupTitle: "आइए आपका खेत सेट करें",
      setupStepOf: function (n, total) { return "चरण " + n + " / " + total; },
      locationQuestion: "आपका खेत कहां है?",
      locationHelp: "इससे हम आपके पास के खतरों की चेतावनी दे पाते हैं।",
      useMyLocation: "मेरा स्थान इस्तेमाल करें",
      locationDetecting: "आपका स्थान ढूंढ रहे हैं…",
      locationDeviceSet: "आपका मौजूदा स्थान इस्तेमाल हो रहा है।",
      locationDistrictSet: function (d) { return d + " पर सेट किया गया।"; },
      locationDenied: "हम आपका स्थान नहीं ले पाए। कृपया नीचे अपना ज़िला चुनें।",
      districtLabel: "या अपना गांव / कस्बा / ज़िला टाइप करें",
      districtPlaceholder: "कोई जगह टाइप करें, जैसे कानपुर",
      searchingPlaces: "खोज रहे हैं…",
      noPlaceMatches: "कोई जगह नहीं मिली। टाइप करते रहें।",
      next: "आगे",
      soilQuestion: "आपकी मिट्टी कैसी है?",
      soilRequired: "कृपया अपनी मिट्टी का प्रकार चुनें।",
      cropsQuestion: "अभी आप क्या उगा रहे हैं?",
      currentCropsHelp: "हर फ़सल और उसकी बुवाई की तारीख जोड़ें।",
      currentCropsLabel: "आपकी फ़सलें",
      addCrop: "फ़सल जोड़ें",
      selectCropPlaceholder: "फ़सल चुनें",
      plantingDateLabel: "बुवाई की तारीख",
      cropRemove: "फ़सल हटाएं",
      cropsRequired: "कृपया कम से कम एक फ़सल जोड़ें।",
      cropDateRequired: "कृपया हर फ़सल की बुवाई की तारीख डालें।",
      finishSetup: "पूरा करें",
      profileTitle: "आपकी प्रोफ़ाइल",
      languageLabel: "भाषा",
      locationLabel: "स्थान",
      saveProfile: "बदलाव सहेजें",
      profileSaved: "प्रोफ़ाइल सहेजी गई।",
      profileSaveError: "हम आपकी प्रोफ़ाइल सहेज नहीं पाए। कृपया फिर से कोशिश करें।",

      remindersTitle: "आपके लिए याद-दिलावे",
      dueToday: "आज करना है",
      reminderIrrigateToday: function (crop) { return crop + " की सिंचाई का समय है।"; },
      reminderIrrigateIn: function (crop, n) { return crop + " की सिंचाई " + n + " दिन में करें।"; },
      reminderIrrigateDrySpell: function (crop) { return "7 दिनों तक बारिश की उम्मीद नहीं — " + crop + " की सिंचाई करें।"; },
      reminderHarvestNow: function (crop) { return crop + " की कटाई का समय है।"; },
      reminderHarvestIn: function (crop, n) { return crop + " की कटाई " + n + " दिन में।"; },
      reminderStageCare: function (crop) { return "आपकी " + crop + " फ़सल:"; },

      homeGreeting: "नमस्ते",
      assistantWelcome: "🙏 नमस्ते — आज मैं आपकी क्या मदद कर सकता हूँ?",
      tapAndSpeak: "बोलने के लिए दबाएं",
      listening: "सुन रहे हैं...",
      micUnavailableHint: "इस फ़ोन में आवाज़ की सुविधा नहीं है। नीचे टेक्स्ट बॉक्स इस्तेमाल करें।",
      micPermissionDenied: "कृपया माइक की अनुमति दें, या नीचे अपना सवाल टाइप करें।",
      voiceNotUnderstood: "माफ़ करें, समझ नहीं आया। फिर से बोलें या नीचे अपना सवाल टाइप करें।",
      diagnoseCardTitle: "फ़सल की जांच करें",
      diagnoseCardSub: "पौधे की फोटो लें",
      assistantQuestionPlaceholder: "जैसे: टमाटर के बाद क्या उगाएं?",
      assistantYouAsked: function (q) { return "आपने पूछा: " + q; },
      assistantAskEmpty: "कृपया पहले एक सवाल पूछें।",
      assistantError: "अभी जवाब नहीं मिल पाया। कृपया फिर से कोशिश करें।",
      chipGrow: "क्या उगाएं?",
      chipMandi: "आज टमाटर का भाव",
      chipFertilizer: "मेरी फ़सल के लिए कौन सी खाद?",
      chipWeather: "क्या इस हफ़्ते बारिश होगी?",

      diagnoseInstruction: "पत्ते की साफ फोटो लें या चुनें",
      addPhoto: "फोटो जोड़ें",
      takePhoto: "फोटो लें",
      uploadPhoto: "अपलोड करें",
      capturePhoto: "फोटो लें",
      cameraLivePreview: "कैमरा लाइव दृश्य",
      photoPreviewAlt: "आपकी चुनी हुई फोटो",
      changePhoto: "दूसरी फोटो चुनें",
      submitDiagnose: "मेरी फ़सल जांचें",
      thinking: "सोच रहे हैं...",
      thinkingSub: "इसमें कुछ पल लग सकते हैं",
      confidenceLow: "कम",
      confidenceMedium: "मध्यम",
      confidenceHigh: "अधिक",
      confidenceUncertain: "पक्का नहीं",
      confidenceLabel: "हमें कितना यकीन है",
      adviceLabel: "क्या करें",
      explainWhat: "यह क्या है",
      explainWhy: "क्यों",
      explainWhatToDo: "क्या करें",
      statusAdvise: "हमें यह मिला",
      statusEscalate: "और जांच ज़रूरी है",
      playAudio: "सुनें",
      newPhoto: "नई फोटो",
      notRight: "यह सही नहीं है",
      disputeConfirmQuestion: "क्या इसे अपने नज़दीकी RSK अधिकारी को दोबारा जांच के लिए भेजें?",
      disputeThanks: "धन्यवाद। हमने आपके नज़दीकी RSK अधिकारी को बता दिया है।",
      diagnoseError: "अभी हम आपकी फोटो जांच नहीं पाए। कृपया फिर से कोशिश करें।",

      recommendInstruction: "अपने खेत के बारे में बताएं",
      soilLabel: "मिट्टी का प्रकार",
      zoneLabel: "आपका इलाका",
      rainfallLabel: "बारिश",
      groundwaterLabel: "पानी की गहराई",
      chooseAllHint: "कृपया हर समूह में एक विकल्प चुनें",
      showRecommendations: "सुझाव देखें",
      bestMatch: "सबसे अच्छा विकल्प",
      startOver: "फिर से शुरू करें",
      recommendResultIntro: "आपके खेत के लिए अच्छी फ़सलें",
      orTypeQuestion: "या अपना सवाल लिखें",
      askBtn: "पूछें",
      askAgain: "फिर से पूछें",

      alertsTitle: "आपकी सूचनाएं",
      alertsEmpty: "अभी कोई सूचना नहीं है। खेत के पास खतरा होने पर हम आपको बताएंगे।",
      alertsLoadError: "सूचनाएं लोड नहीं हो पाईं। कृपया इंटरनेट जांचें।",
      inYourArea: "आपके इलाके में",
      withinKm: function (km) { return km + " किमी के अंदर"; },
      today: "आज",
      yesterday: "कल",
      daysAgo: function (n) { return n + " दिन पहले"; },
      alertsSummarySpoken: function (n) {
        return n === 1 ? "आपके लिए 1 नई सूचना है" : "आपके लिए " + n + " नई सूचनाएं हैं";
      },

      reportsTitle: "आपकी रिपोर्ट",
      reportsEmpty: "आपने अभी तक किसी फ़सल की जांच नहीं की है।",
      reportsLoadError: "रिपोर्ट लोड नहीं हो पाईं। कृपया इंटरनेट जांचें।",
      officerReviewed: "RSK अधिकारी द्वारा जांचा गया",
      reportStatuses: {
        pending: "जांच हो रही है...",
        advised: "AI जांच",
        escalated: "अधिकारी की समीक्षा बाकी है",
        disputed: "आपने इसे फ़्लैग किया",
        confirmed: "अधिकारी द्वारा पुष्टि की गई"
      },
      verdictModalTitle: "एक अधिकारी ने आपकी फ़सल की जांच की",
      verdictModalBody: function (condition) {
        return "स्थानीय RSK अधिकारी ने आपके मामले की जांच की है। उनका नतीजा: " + condition + "।";
      },
      verdictModalBodyMany: function (n) {
        return "स्थानीय RSK अधिकारी ने आपके " + n + " मामलों की जांच की है। विवरण देखने के लिए नीचे टैप करें।";
      },
      verdictModalView: "विवरण देखें",
      verdictModalDismiss: "बंद करें",

      conditions: {
        late_blight: "पछेती झुलसा (Late Blight)",
        early_blight: "अगेती झुलसा (Early Blight)",
        nitrogen_deficiency: "नाइट्रोजन की कमी",
        other: "अन्य समस्या",
        healthy: "स्वस्थ फ़सल"
      },
      crops: {
        rice: "धान", tomato: "टमाटर", chili: "मिर्च",
        cotton: "कपास", groundnut: "मूंगफली", maize: "मक्का"
      },
      reasons: {
        high_rainfall_shallow_water_suits_paddy: "अच्छी बारिश और कम गहरे पानी में यह फ़सल अच्छी होती है",
        moderate_water_well_drained_soil_suits_tomato: "अच्छे जल निकास वाली मिट्टी में यह फ़सल अच्छी होती है",
        well_drained_soil_moderate_rainfall_suits_chili: "अच्छे जल निकास और मध्यम बारिश में यह फ़सल अच्छी होती है",
        black_soil_semi_arid_zone_suits_cotton: "सूखे इलाके की काली मिट्टी में यह फ़सल अच्छी होती है",
        low_water_need_suits_deep_groundwater_zones: "इसे कम पानी चाहिए, आपके इलाके के लिए अच्छी है",
        adaptable_crop_moderate_rainfall_suits_maize: "यह आसान फ़सल है, मध्यम बारिश में अच्छी होती है"
      },
      soils: {
        alluvial: "जलोढ़ (नदी की) मिट्टी", black: "काली मिट्टी", loamy: "दोमट मिट्टी",
        loam: "दोमट मिट्टी", clay: "चिकनी मिट्टी",
        red: "लाल मिट्टी", sandy: "रेतीली मिट्टी"
      },
      zones: { delta: "डेल्टा क्षेत्र", coastal: "तटीय क्षेत्र", upland: "ऊंचा इलाका", semi_arid: "सूखा इलाका" },
      rainfall: { low: "कम बारिश", medium: "मध्यम बारिश", high: "ज़्यादा बारिश" },
      groundwater: { shallow: "कम गहरा", medium: "मध्यम", deep: "ज़्यादा गहरा" },
      tiers: { watch: "नज़र रखें", warning: "चेतावनी", alert: "खतरा" },

      demoSectionTitle: "डेमो",
      resetDemo: "डेमो डेटा रीसेट करें",
      resetDemoDone: "डेमो डेटा साफ़ स्थिति में रीसेट हो गया।",
      resetDemoError: "डेमो डेटा रीसेट नहीं हो पाया। कृपया फिर से कोशिश करें।",

      skipIntro: "इंट्रो छोड़ें",
      watchIntro: "इंट्रो देखें",
      officerSignIn: "RSK अधिकारी साइन इन",
      introWarned: "चेतावनी मिली",
      introSafe: "ख़तरे में नहीं",
      introDxNote: "फोटो में शुरुआती धब्बे दिखते हैं, पर ठंडा-नम मौसम पछेती झुलसा की ओर इशारा करता है — दो अलग बीमारियाँ। अंदाज़ा लगाने के बजाय KisanMate इसे स्थानीय अधिकारी को भेजता है।",
      introScene1: "ये हैं रमेश, गुंटूर के एक टमाटर किसान।",
      introScene2: "उनकी फ़सल बीमार लग रही है, इसलिए वे एक फोटो भेजते हैं।",
      introScene3: "KisanMate दो बीमारियों के बीच पक्का नहीं है, इसलिए अंदाज़ा लगाने के बजाय यह मामला स्थानीय अधिकारी को भेजता है।",
      introScene4: "एक कृषि अधिकारी इसे देखकर पछेती झुलसा की पुष्टि करते हैं।",
      introScene5: "क्योंकि यह फैलती है, पास के टमाटर किसानों को तुरंत चेतावनी मिलती है। पड़ोस के धान किसान को नहीं — सिर्फ़ उन्हें जो सच में ख़तरे में हैं।",
      introScene6: "यह है KisanMate। चलिए शुरू करें।"
    },

    te: {
      appName: "KisanMate",
      tagline: "మీ వ్యవసాయ సహాయకుడు",
      skipToContent: "ప్రధాన విషయానికి వెళ్ళండి",
      changeLanguage: "భాష మార్చండి",
      muteVoice: "వాయిస్ సమాధానాలు ఆపు",
      unmuteVoice: "వాయిస్ సమాధానాలు ఆన్ చేయి",
      govtSchemes: "ప్రభుత్వ పథకాలు",
      govtSchemesMenuLabel: "ప్రభుత్వ పథకాల మెనూ",
      govtSchemePmKisan: "పీఎం-కిసాన్ (కిసాన్ నిధి)",
      govtSchemePmfby: "పంట బీమా (కిసాన్ బీమా)",
      govtSchemeSoilHealth: "నేల ఆరోగ్య కార్డు",
      back: "వెనుకకు",
      navHome: "హోమ్",
      navDiagnose: "పరీక్ష",
      navReports: "నివేదికలు",
      navAlerts: "హెచ్చరికలు",
      retry: "మళ్ళీ ప్రయత్నించండి",
      cancel: "రద్దు చేయండి",
      loading: "లోడ్ అవుతోంది...",
      friendlyErrorGeneric: "ఏదో తప్పు జరిగింది. దయచేసి మళ్ళీ ప్రయత్నించండి.",
      friendlyErrorSlow: "సాధారణం కంటే ఎక్కువ సమయం పడుతోంది. దయచేసి ఇంటర్నెట్ చూసి మళ్ళీ ప్రయత్నించండి.",

      getStarted: "ప్రారంభించండి",
      continueLabel: "కొనసాగండి",
      continueBtn: "హోమ్‌కు వెళ్ళండి",
      signOut: "సైన్ అవుట్",
      signedOut: "మీరు సైన్ అవుట్ అయ్యారు.",
      sessionExpired: "మీ సెషన్ చెల్లదు. దయచేసి మళ్ళీ సైన్ ఇన్ చేయండి.",
      phoneTitle: "మీ ఫోన్‌తో సైన్ ఇన్ చేయండి",
      phoneInstruction: "మీ ఫోన్ నంబర్ నమోదు చేయండి, మేము మీకు కోడ్ పంపుతాము.",
      phoneLabel: "ఫోన్ నంబర్",
      phonePlaceholder: "10 అంకెల మొబైల్ నంబర్",
      phoneInvalid: "దయచేసి మీ 10 అంకెల మొబైల్ నంబర్ నమోదు చేయండి.",
      sendCode: "కోడ్ పంపండి",
      otpRequestError: "ప్రస్తుతం కోడ్ పంపలేకపోయాము. దయచేసి మళ్ళీ ప్రయత్నించండి.",
      otpTitle: "కోడ్ నమోదు చేయండి",
      otpSentTo: function (phone) { return "మేము " + phone + " కు కోడ్ పంపాము."; },
      demoOtpLabel: "డెమో OTP",
      otpLabel: "4 అంకెల కోడ్",
      otpInvalid: "ఈ కోడ్ సరిపోలడం లేదు. దయచేసి మళ్ళీ ప్రయత్నించండి.",
      verifyCode: "నిర్ధారించి కొనసాగండి",
      changeNumber: "నంబర్ మార్చండి",
      profileSetupTitle: "మీ ప్రొఫైల్‌ను సెటప్ చేయండి",
      profileSetupIntro: "స్వాగతం! ప్రారంభించడానికి మీ పేరు చెప్పండి. మీ పొలం వివరాలను తర్వాత జోడించవచ్చు.",
      profileNameLabel: "మీ పేరు",
      profileNamePlaceholder: "ఉదా. రమేష్",

      setupTitle: "మీ పొలాన్ని సెటప్ చేద్దాం",
      setupStepOf: function (n, total) { return "అడుగు " + n + " / " + total; },
      locationQuestion: "మీ పొలం ఎక్కడ ఉంది?",
      locationHelp: "మీ దగ్గర ఉన్న ప్రమాదాల గురించి హెచ్చరించడానికి దీన్ని వాడతాము.",
      useMyLocation: "నా లొకేషన్ వాడండి",
      locationDetecting: "మీ లొకేషన్ కనుగొంటున్నాం…",
      locationDeviceSet: "మీ ప్రస్తుత లొకేషన్ వాడుతున్నాం.",
      locationDistrictSet: function (d) { return d + " కు సెట్ చేయబడింది."; },
      locationDenied: "మేము మీ లొకేషన్ పొందలేకపోయాము. దయచేసి కింద మీ జిల్లాను ఎంచుకోండి.",
      districtLabel: "లేదా మీ ఊరు / పట్టణం / జిల్లా టైప్ చేయండి",
      districtPlaceholder: "ఒక ప్రదేశం టైప్ చేయండి, ఉదా. కాన్పూర్",
      searchingPlaces: "వెతుకుతోంది…",
      noPlaceMatches: "సరిపోలే ప్రదేశాలు లేవు. టైప్ చేస్తూ ఉండండి.",
      next: "తర్వాత",
      soilQuestion: "మీ నేల ఎలా ఉంటుంది?",
      soilRequired: "దయచేసి మీ నేల రకాన్ని ఎంచుకోండి.",
      cropsQuestion: "ఇప్పుడు మీరు ఏమి పండిస్తున్నారు?",
      currentCropsHelp: "ప్రతి పంటను, దాన్ని నాటిన తేదీని జోడించండి.",
      currentCropsLabel: "మీ పంటలు",
      addCrop: "పంట జోడించండి",
      selectCropPlaceholder: "పంటను ఎంచుకోండి",
      plantingDateLabel: "నాటిన తేదీ",
      cropRemove: "పంటను తీసివేయండి",
      cropsRequired: "దయచేసి కనీసం ఒక పంటను జోడించండి.",
      cropDateRequired: "దయచేసి ప్రతి పంటకు నాటిన తేదీని పెట్టండి.",
      finishSetup: "పూర్తి చేయండి",
      profileTitle: "మీ ప్రొఫైల్",
      languageLabel: "భాష",
      locationLabel: "లొకేషన్",
      saveProfile: "మార్పులు సేవ్ చేయండి",
      profileSaved: "ప్రొఫైల్ సేవ్ అయింది.",
      profileSaveError: "మేము మీ ప్రొఫైల్‌ను సేవ్ చేయలేకపోయాము. దయచేసి మళ్ళీ ప్రయత్నించండి.",

      remindersTitle: "మీ కోసం రిమైండర్లు",
      dueToday: "ఈరోజు చేయాలి",
      reminderIrrigateToday: function (crop) { return "మీ " + crop + " కి నీరు పెట్టే సమయం."; },
      reminderIrrigateIn: function (crop, n) { return "మీ " + crop + " కి " + n + " రోజుల్లో నీరు పెట్టండి."; },
      reminderIrrigateDrySpell: function (crop) { return "7 రోజులు వర్షం పడే అవకాశం లేదు — మీ " + crop + " కి నీరు పెట్టండి."; },
      reminderHarvestNow: function (crop) { return "మీ " + crop + " కోత సమయం."; },
      reminderHarvestIn: function (crop, n) { return "మీ " + crop + " కోత " + n + " రోజుల్లో."; },
      reminderStageCare: function (crop) { return "మీ " + crop + ":"; },

      homeGreeting: "నమస్కారం",
      assistantWelcome: "🙏 నమస్తే — ఈరోజు నేను మీకు ఎలా సహాయం చేయగలను?",
      tapAndSpeak: "మాట్లాడటానికి నొక్కండి",
      listening: "వింటున్నాం...",
      micUnavailableHint: "ఈ ఫోన్‌లో వాయిస్ సదుపాయం లేదు. కింద టెక్స్ట్ బాక్స్ వాడండి.",
      micPermissionDenied: "దయచేసి మైక్‌కి అనుమతి ఇవ్వండి, లేదా కింద మీ ప్రశ్న టైప్ చేయండి.",
      voiceNotUnderstood: "క్షమించండి, అర్థం కాలేదు. మళ్ళీ చెప్పండి లేదా కింద మీ ప్రశ్న టైప్ చేయండి.",
      diagnoseCardTitle: "పంటను పరీక్షించండి",
      diagnoseCardSub: "మొక్క ఫోటో తీయండి",
      assistantQuestionPlaceholder: "ఉదా. టమాటా తర్వాత ఏమి పండించాలి?",
      assistantYouAsked: function (q) { return "మీరు అడిగారు: " + q; },
      assistantAskEmpty: "దయచేసి ముందు ఒక ప్రశ్న అడగండి.",
      assistantError: "ప్రస్తుతం సమాధానం దొరకలేదు. దయచేసి మళ్ళీ ప్రయత్నించండి.",
      chipGrow: "ఏమి పండించాలి?",
      chipMandi: "ఈరోజు టమాటా ధర",
      chipFertilizer: "నా పంటకు ఏ ఎరువు?",
      chipWeather: "ఈ వారం వర్షం పడుతుందా?",

      diagnoseInstruction: "ఆకు యొక్క స్పష్టమైన ఫోటో తీయండి లేదా ఎంచుకోండి",
      addPhoto: "ఫోటో జోడించండి",
      takePhoto: "ఫోటో తీయండి",
      uploadPhoto: "అప్‌లోడ్ చేయండి",
      capturePhoto: "ఫోటో తీయండి",
      cameraLivePreview: "కెమెరా ప్రత్యక్ష ప్రివ్యూ",
      photoPreviewAlt: "మీరు ఎంచుకున్న ఫోటో",
      changePhoto: "వేరే ఫోటో ఎంచుకోండి",
      submitDiagnose: "నా పంటను పరీక్షించండి",
      thinking: "ఆలోచిస్తున్నాం...",
      thinkingSub: "దీనికి కొన్ని క్షణాలు పట్టవచ్చు",
      confidenceLow: "తక్కువ",
      confidenceMedium: "మధ్యస్థం",
      confidenceHigh: "ఎక్కువ",
      confidenceUncertain: "ఖచ్చితంగా తెలియదు",
      confidenceLabel: "మాకు ఎంత నమ్మకం ఉంది",
      adviceLabel: "ఏమి చేయాలి",
      explainWhat: "ఇది ఏమిటి",
      explainWhy: "ఎందుకు",
      explainWhatToDo: "ఏమి చేయాలి",
      statusAdvise: "మాకు ఇది కనిపించింది",
      statusEscalate: "మరింత పరిశీలన అవసరం",
      playAudio: "వినండి",
      newPhoto: "కొత్త ఫోటో",
      notRight: "ఇది సరైనది కాదు",
      disputeConfirmQuestion: "దీన్ని మీ సమీప RSK అధికారికి మళ్ళీ చూడటానికి పంపాలా?",
      disputeThanks: "ధన్యవాదాలు. మేము మీ సమీప RSK అధికారికి తెలియజేశాము.",
      diagnoseError: "ప్రస్తుతం మేము మీ ఫోటోను పరీక్షించలేకపోయాము. దయచేసి మళ్ళీ ప్రయత్నించండి.",

      recommendInstruction: "మీ పొలం గురించి చెప్పండి",
      soilLabel: "నేల రకం",
      zoneLabel: "మీ ప్రాంతం",
      rainfallLabel: "వర్షపాతం",
      groundwaterLabel: "నీటి లోతు",
      chooseAllHint: "దయచేసి ప్రతి గుంపులో ఒక ఎంపిక చేయండి",
      showRecommendations: "సలహాలు చూడండి",
      bestMatch: "ఉత్తమ ఎంపిక",
      startOver: "మళ్ళీ మొదలు పెట్టండి",
      recommendResultIntro: "మీ పొలానికి మంచి పంటలు",
      orTypeQuestion: "లేదా మీ ప్రశ్న టైప్ చేయండి",
      askBtn: "అడగండి",
      askAgain: "మళ్ళీ అడగండి",

      alertsTitle: "మీ హెచ్చరికలు",
      alertsEmpty: "ప్రస్తుతం హెచ్చరికలు లేవు. మీ పొలం దగ్గర ప్రమాదం ఉంటే మేము మీకు తెలియజేస్తాము.",
      alertsLoadError: "హెచ్చరికలు లోడ్ కాలేదు. దయచేసి ఇంటర్నెట్ చూడండి.",
      inYourArea: "మీ ప్రాంతంలో",
      withinKm: function (km) { return km + " కి.మీ. లోపల"; },
      today: "ఈరోజు",
      yesterday: "నిన్న",
      daysAgo: function (n) { return n + " రోజుల క్రితం"; },
      alertsSummarySpoken: function (n) {
        return n === 1 ? "మీకు 1 కొత్త హెచ్చరిక ఉంది" : "మీకు " + n + " కొత్త హెచ్చరికలు ఉన్నాయి";
      },

      reportsTitle: "మీ నివేదికలు",
      reportsEmpty: "మీరు ఇంకా ఏ పంటనూ పరీక్షించలేదు.",
      reportsLoadError: "నివేదికలు లోడ్ కాలేదు. దయచేసి ఇంటర్నెట్ చూడండి.",
      officerReviewed: "RSK అధికారి సమీక్షించారు",
      reportStatuses: {
        pending: "పరిశీలిస్తోంది...",
        advised: "AI నిర్ధారణ",
        escalated: "అధికారి సమీక్ష కోసం వేచి ఉంది",
        disputed: "మీరు దీన్ని ఫ్లాగ్ చేశారు",
        confirmed: "అధికారి నిర్ధారించారు"
      },
      verdictModalTitle: "ఒక అధికారి మీ పంటను సమీక్షించారు",
      verdictModalBody: function (condition) {
        return "స్థానిక RSK అధికారి మీ కేసును సమీక్షించారు. వారి నిర్ధారణ: " + condition + ".";
      },
      verdictModalBodyMany: function (n) {
        return "స్థానిక RSK అధికారి మీ " + n + " కేసులను సమీక్షించారు. వివరాల కోసం కింద నొక్కండి.";
      },
      verdictModalView: "వివరాలు చూడండి",
      verdictModalDismiss: "మూసివేయి",

      conditions: {
        late_blight: "ఆలస్య తెగులు (Late Blight)",
        early_blight: "త్వరిత తెగులు (Early Blight)",
        nitrogen_deficiency: "నత్రజని లోపం",
        other: "ఇతర సమస్య",
        healthy: "ఆరోగ్యకరమైన పంట"
      },
      crops: {
        rice: "వరి", tomato: "టమాటా", chili: "మిర్చి",
        cotton: "పత్తి", groundnut: "వేరుశనగ", maize: "మొక్కజొన్న"
      },
      reasons: {
        high_rainfall_shallow_water_suits_paddy: "మంచి వర్షం మరియు తక్కువ లోతు నీరు ఈ పంటకు బాగుంటుంది",
        moderate_water_well_drained_soil_suits_tomato: "నీరు త్వరగా ఇంకే నేలలో ఈ పంట బాగా పండుతుంది",
        well_drained_soil_moderate_rainfall_suits_chili: "నీరు ఇంకే నేల మరియు మధ్యస్థ వర్షంలో ఈ పంట బాగుంటుంది",
        black_soil_semi_arid_zone_suits_cotton: "పొడి ప్రాంతంలోని నల్ల నేలలో ఈ పంట బాగుంటుంది",
        low_water_need_suits_deep_groundwater_zones: "దీనికి నీరు తక్కువ కావాలి, మీ ప్రాంతానికి బాగుంటుంది",
        adaptable_crop_moderate_rainfall_suits_maize: "ఇది సులభమైన పంట, మధ్యస్థ వర్షంలో బాగా పండుతుంది"
      },
      soils: {
        alluvial: "ఒండ్రు (నది) నేల", black: "నల్ల నేల", loamy: "గరప నేల (Loamy)",
        loam: "గరప నేల", clay: "బంకమట్టి నేల",
        red: "ఎర్ర నేల", sandy: "ఇసుక నేల"
      },
      zones: { delta: "డెల్టా ప్రాంతం", coastal: "తీర ప్రాంతం", upland: "ఎత్తైన ప్రాంతం", semi_arid: "పొడి ప్రాంతం" },
      rainfall: { low: "తక్కువ వర్షం", medium: "మధ్యస్థ వర్షం", high: "ఎక్కువ వర్షం" },
      groundwater: { shallow: "తక్కువ లోతు", medium: "మధ్యస్థం", deep: "ఎక్కువ లోతు" },
      tiers: { watch: "గమనించండి", warning: "హెచ్చరిక", alert: "ప్రమాదం" },

      demoSectionTitle: "డెమో",
      resetDemo: "డెమో డేటా రీసెట్ చేయి",
      resetDemoDone: "డెమో డేటా శుభ్రమైన స్థితికి రీసెట్ అయింది.",
      resetDemoError: "డెమో డేటా రీసెట్ చేయలేకపోయాము. దయచేసి మళ్ళీ ప్రయత్నించండి.",

      skipIntro: "ఇంట్రో దాటవేయి",
      watchIntro: "ఇంట్రో చూడండి",
      officerSignIn: "RSK అధికారి సైన్ ఇన్",
      introWarned: "హెచ్చరిక అందింది",
      introSafe: "ప్రమాదంలో లేదు",
      introDxNote: "ఫోటోలో తొలి మచ్చలు కనిపిస్తున్నాయి, కానీ చల్లని తడి వాతావరణం ఆలస్య తెగులును సూచిస్తోంది — రెండు వేర్వేరు వ్యాధులు. ఊహించకుండా KisanMate దీన్ని స్థానిక అధికారికి పంపుతుంది.",
      introScene1: "ఇతను రమేష్, గుంటూరులో టమాటా రైతు.",
      introScene2: "అతని పంట అనారోగ్యంగా కనిపిస్తోంది, అందుకే ఫోటో పంపుతాడు.",
      introScene3: "KisanMate రెండు వ్యాధుల మధ్య ఖచ్చితంగా చెప్పలేకపోతోంది, కాబట్టి ఊహించకుండా కేసును స్థానిక అధికారికి పంపుతుంది.",
      introScene4: "ఒక వ్యవసాయ అధికారి దీన్ని సమీక్షించి ఆలస్య తెగులు అని నిర్ధారిస్తారు.",
      introScene5: "ఇది వ్యాపిస్తుంది కాబట్టి, దగ్గర్లోని టమాటా రైతులకు వెంటనే హెచ్చరిక వెళ్తుంది. పక్కనే ఉన్న వరి రైతుకు కాదు — నిజంగా ప్రమాదంలో ఉన్నవారికే.",
      introScene6: "ఇదే KisanMate. మొదలుపెడదాం."
    }
  };

  global.KM_STRINGS = STRINGS;
  global.KM_LOCALE_TAGS = LOCALE_TAGS;
  global.KM_LANGUAGES = LANGUAGES;
  global.KM_CHOOSE_LANGUAGE_HEADING = CHOOSE_LANGUAGE_HEADING;
})(window);
